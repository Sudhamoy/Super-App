...
function Movies() {
  const selectedMoviesByUsers=JSON.parse(localStorage.getItem("selectedMovies"))
  //[Action,Drama,Romance]->iterating over these
  
  const selectedGenresToFetch=selectedMoviesByUsers.map((movie)=>{
    return GENRE_LIST.find((genre)=>genre.name===movie)
  })
  console.log(selectedMoviesByUsers)
...
try {
  ...
  return result
    ...
    /* IN CASE OF "RATE LIMIT" In Place of Promise.all:
  promises.forEach(async (promise)=>{
    const data=await promise
    setTimeout(()=>{
      setMovies([...movies,...JSON.parse(data)]) //((prev)=>[...prev,JSON.parse(data)])
    },5000)
  })
    setMovies(
    [...movies,...selectedMoviesByUsers] //((prev)=>[...prev,JSON.parse(data)])
   ) */
  ...
  return (
    ...
    <h1 className="text-green-500">Super App</h1>
    <h1 className="text-white">Entertainment according to your choice</h1>
    <div className="flex flex-wrap">
    {movies.map((movie)=>{
        return (
        <div key={movie.id} className="border-solid border-2 border-sky-500 m-2 h-48 w-60">
